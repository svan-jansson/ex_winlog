defmodule ExWinlog.Logger do
  ...
  def register(event_source_name) do
    ...
    event_message_file =
      (Application.app_dir(:ex_winlog, "priv/native") <> "/ex_winlog_nif.dll")
      |> String.replace("/", "\\")
      |> String.to_charlist()

    with {:ok, :event_source_registered} <- Nif.register(event_source_name),
         {:ok, reg_handle} <- :win32reg.open([:read, :write]) do
      try do
        with :ok <- :win32reg.change_key(reg_handle, reg_key),
             :ok <- :win32reg.set_value(reg_handle, "EventMessageFile", event_message_file) do
          {:ok, :event_source_registered}
        end
      after
        :win32reg.close(reg_handle)
      end
    end
  end
  ...
  def warning(event_source_name, log_string), do: Nif.warn(event_source_name, log_string)
